...
namespace {
   ...
   test_case<> ANON(
	 ...
	 be.add_field( "diskscaleradius" );
         ...
         TEST( res == "SELECT coldgas AS coldgas, diskscaleradius AS diskscaleradius, globalindex AS globalindex, globaltreeid AS globaltreeid, localgalaxyid AS localgalaxyid, metalscoldgas AS metalscoldgas, (posx + 0 - 0) AS posx, (posy + 0 - 0) AS posy, (posz + 0 - 0) AS posz, sfr AS sfr, snapnum AS snapnum, velx AS velx, vely AS vely, velz AS velz FROM -table- WHERE snapnum = 3 AND (posx + 0 - 0) > 0 AND (posx + 0 - 0) < 10 AND (posy + 0 - 0) > 0 AND (posy + 0 - 0) < 10 AND (posz + 0 - 0) > 0 AND (posz + 0 - 0) < 10" );
	 ...
	 TEST( res == "SELECT coldgas AS coldgas, diskscaleradius AS diskscaleradius, globalindex AS globalindex, globaltreeid AS globaltreeid, localgalaxyid AS localgalaxyid, metalscoldgas AS metalscoldgas, (posx + 0 - 1) AS posx, (posy + 0 - 2) AS posy, (posz + 0 - 3) AS posz, sfr AS sfr, snapnum AS snapnum, velx AS velx, vely AS vely, velz AS velz FROM -table- WHERE snapnum = 3 AND (posx + 0 - 1) > 0 AND (posx + 0 - 1) < 10 AND (posy + 0 - 2) > 0 AND (posy + 0 - 2) < 10 AND (posz + 0 - 3) > 0 AND (posz + 0 - 3) < 10" );
   ...
   test_case<> ANON(
      "/tao/base/rdb_backend/make_tile_query_string",
      "",
      []()
      {
         dummy be;
	 ...
	 TEST( res == "SELECT coldgas AS coldgas, diskscaleradius AS diskscaleradius, globalindex AS globalindex, globaltreeid AS globaltreeid, localgalaxyid AS localgalaxyid, metalscoldgas AS metalscoldgas, (posx + 0 - 1) AS posx, (posy + 0 - 2) AS posy, (posz + 0 - 3) AS posz, sfr AS sfr, snapnum AS snapnum, velx AS velx, vely AS vely, velz AS velz FROM -table- INNER JOIN redshift_ranges ON (-table-.snapnum = redshift_ranges.snapshot) WHERE (POW((posx + 0 - 1),2) + POW((posy + 0 - 2),2) + POW((posz + 0 - 3),2)) >= redshift_ranges.min AND (POW((posx + 0 - 1),2) + POW((posy + 0 - 2),2) + POW((posz + 0 - 3),2)) < redshift_ranges.max AND ATAN2((posy + 0 - 2),(posx + 0 - 1)) >= 0 AND ATAN2((posy + 0 - 2),(posx + 0 - 1)) < 0.174532925199 AND (0.5*PI() - ACOS((posz + 0 - 3)/(SQRT(POW((posx + 0 - 1),2) + POW((posy + 0 - 2),2) + POW((posz + 0 - 3),2))))) >= 0 AND (0.5*PI() - ACOS((posz + 0 - 3)/(SQRT(POW((posx + 0 - 1),2) + POW((posy + 0 - 2),2) + POW((posz + 0 - 3),2))))) < 0.174532925199 AND (POW((posx + 0 - 1),2) + POW((posy + 0 - 2),2) + POW((posz + 0 - 3),2)) >= 0 AND (POW((posx + 0 - 1),2) + POW((posy + 0 - 2),2) + POW((posz + 0 - 3),2)) < 31598.4212922" );
