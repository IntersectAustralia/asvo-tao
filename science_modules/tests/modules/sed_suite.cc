...
typedef tao::modules::sed<tao::backends::soci<tao::real_type>> sed_type;

TEST_CASE( "/tao/modules/sed/default_constructor" )
{
   sed_type sed;
   TEST( sed.backend() == (void*)0 );
   TEST( sed.simulation() == (void*)0 );
   TEST( sed.disk_age_masses().empty() == true );
   TEST( sed.bulge_age_masses().empty() == true );
}

TEST_CASE( "/tao/modules/sed/disk" )
{
   sed_type sed;
}

TEST_CASE( "/tao/modules/sed/bulge" )
{
   sed_type sed;
}

TEST_CASE( "/tao/modules/sed/total" )
{
   sed_type sed;
}
