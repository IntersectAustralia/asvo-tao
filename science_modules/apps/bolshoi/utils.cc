...
///
void
load_forests_map( multimap<long long,long long>& forests )
{
   LOG_ENTER();
...
///
void
load_locations_map( map<long long,std::pair<unsigned,size_t>>& locations,
...
///
void
load_forest_sizes_map( map<long long,unsigned>& forest_sizes )
{
   LOG_ENTER();
   ...
   // Open the gzip stream.
   std::ifstream file( "forest_sizes.dat", std::ios_base::in );
   ASSERT( file );
