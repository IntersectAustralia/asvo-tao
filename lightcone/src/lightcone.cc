...
namespace tao {
   ...
   void
   _build_pixels( mpi::lindex cur_snap_idx,
                  optional<mpi::lindex> next_snap_idx,
                  real_type offs_x,
                  real_type offs_y,
                  real_type offs_z )
   {
      auto query = _get_galaxies( cur_snap_idx, next_snap_idx, offs_x, offs_y, offs_z );

      // TODO: Fetch the query and output each row.
   }

   void
   _get_galaxies( mpi::lindex cur_snap_idx,
                  mpi::lindex next_snap_idx,
                  real_type offs_x,
                  real_type offs_y,
                  real_type offs_z )
   {
      ...
      auto rotation_array = _random_rotation_and_shifting();
