...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        objs = orm.GlobalParameter.objects.filter(parameter_name='INITIAL_JOB_STATUS')
        if len(objs) == 0:
            obj = orm.GlobalParameter(parameter_name='INITIAL_JOB_STATUS', parameter_value='HELD',description='Status assigned to jobs when created in New Catalogue. Use one of HELD or SUBMITTED.')
            obj.save()
