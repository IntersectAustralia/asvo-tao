...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        parameter_name = 'job_too_large_warning'
        objs = orm.GlobalParameter.objects.filter(parameter_name=parameter_name)
        if len(objs) == 0:
            obj = orm.GlobalParameter(
                parameter_name=parameter_name,
                parameter_value='<i><em>NOTE:</em> This job may not complete within the allowed time.</i>',
                description='Warning text for the UI next to calculated job size. HTML is allowed, use with care.'
                )
            obj.save()
