      ...
      char* argv[] )
{
   ...
   vector<size_t> forest_tree_displs;
   ...
   vector<string> location_files;
   {
      ...
      aux_file.reada<size_t>( "forest_tree_displs", forest_tree_displs );
      aux_file.reada<unsigned>( "forest_halo_cnts", forest_halo_cnts );
      aux_file.reada<size_t>( "location_file_offsets", location_file_offsets );
      aux_file.reada<unsigned>( "location_file_idxs", location_file_idxs );
...
#ifndef NDEBUG
   // Check sizes.
   ASSERT( forest_tree_displs.size() == forest_halo_cnts.size() + 1 );
#endif
...
#pragma omp parallel
   {
	 ...
	 unsigned num_trees = forest_tree_displs[ii + 1] - tree_displ;
